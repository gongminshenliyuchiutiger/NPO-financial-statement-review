...
function setTranslate(xPos, yPos, el) {
    // Combine transform with existing animation if needed, 
    // but here we just set translate. 
    // Note: This overrides the 'shake' animation transform property if active.
    // To fix, we wrap the image in a container or use top/left.
}

// Better approach for dragging without conflicting with CSS transform (shake)
// Rebooting drag logic to use top/left style
mascot.removeEventListener("mousedown", dragStart);
document.removeEventListener("mouseup", dragEnd);
document.removeEventListener("mousemove", drag);

// New Drag Logic
let dragItem = document.getElementById("mascot");
let active = false;
let currentX_new, currentY_new, initialX_new, initialY_new;
let xOffset_new = 0;
let yOffset_new = 0;

dragItem.addEventListener("mousedown", dragStart_new, false);
document.addEventListener("mouseup", dragEnd_new, false);
document.addEventListener("mousemove", drag_new, false);

function dragStart_new(e) {
    initialX_new = e.clientX - xOffset_new;
    initialY_new = e.clientY - yOffset_new;
...
function dragEnd_new(e) {
    initialX_new = currentX_new;
    initialY_new = currentY_new;
    active = false;
}
...
function drag_new(e) {
    if (active) {
        ...
        currentX_new = e.clientX - initialX_new;
        currentY_new = e.clientY - initialY_new;

        xOffset_new = currentX_new;
        yOffset_new = currentY_new;

        setTranslate_new(currentX_new, currentY_new, dragItem);
    }
}

function setTranslate_new(xPos, yPos, el) {
